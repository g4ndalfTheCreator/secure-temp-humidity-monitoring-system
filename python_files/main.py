...
DatabaseLogger = pg.DatabaseLogger(
    dbname=dbname,
    user=psql_user,
...
def handle_callback(topic, payload):
    ...
    if latest_temperature is not None and latest_humidity is not None:
        ...
        print(
            f"Received temperature: {latest_temperature}, humidity: {latest_humidity}"
        )
        DatabaseLogger.log_data_to_db(
            temperature=latest_temperature, humidity=latest_humidity, sensor=sensor
        )
        latest_temperature = None
        latest_humidity = None
